...
def double_pendulum_deriv(this_state, time_step, torques, dampings):
    ...
    m_mat = np.matrix([[a + (2 * b * cos(θ2)), d + (b * cos(θ2))], [d + (b * cos(θ2)), d]])

    torque_mat = np.matrix([[torques[0] - dampings[0] * ω1],
                   [torques[1] - dampings[1] * ω2]])

    b_mat = np.matrix([[-b * sin(θ2) * ω2, -b * sin(θ1 + θ2)],
                   [b * sin(θ2) * ω1, 0]])

    speed_mat = np.matrix([[ω1], [ω2]])

    gravity_mat = 

    ω1_dot, ω2_dot = np.linalg.solve(m_mat, torque_mat - np.matmul(b_mat, speed_mat))
